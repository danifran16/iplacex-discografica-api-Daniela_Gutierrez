...
@PostMapping(
  ...
  public ResponseEntity<Disco> HandleGetDiscoRequest(@PathVariable("id") String id){
    ...
    return new ResponseEntity<>(discoTemp.get(), null, HttpStatus.OK);
  }
  ...
  public ResponseEntity<List<Disco>> HandleGetDiscosByArtistaRequest(
    ...
    List<Disco> discoArt =  discoRepo.findDiscosByIdArtista(id);
    
    if(discoArt.isEmpty()){
      return new ResponseEntity<>( null, HttpStatus.NOT_FOUND);
    }

    return new ResponseEntity<>(discoArt, null, HttpStatus.OK);
  }
